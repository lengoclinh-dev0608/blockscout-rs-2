...
mod tests {
    ...
    async fn cache_view_works(pool: PgPool) -> anyhow::Result<()> {
        ...
        conn.execute("CREATE SCHEMA sgd3;").await?;
        conn.execute("CREATE TABLE sgd3.foo(bar integer)").await?;
        conn.execute("INSERT INTO sgd3.foo VALUES (1)").await?;

        TestView::create_view(&pool, "sgd3").await?;
        assert_current_max_is(&pool, 1).await;
        conn.execute("INSERT INTO sgd3.foo VALUES (100)").await?;
        assert_current_max_is(&pool, 1).await;
        TestView::refresh_view(&pool, "sgd3").await?;
        assert_current_max_is(&pool, 100).await;
        Ok(())
    }

    async fn assert_current_max_is(pool: &PgPool, max_int: i32) {
        let row = sqlx::query("SELECT * FROM sgd3.test_view")
            .fetch_one(pool)
            .await
